...
from tauto_slm.provider import SlmProviderRef
...
def _ctx(cs: ContractSet):
    return build_deterministic_context(cs, generator_intent="lean_verification")
...
def test_traceability_carries_all_metadata() -> None:
    ...
    t = build_traceability(
        contract_set_hash=ctx.entries["contract_set_hash"],
        deterministic_context_hash=ctx.context_hash,
        provider=_provider(),
...
def test_traceability_allows_no_provider_for_deterministic_generators() -> None:
    cs = _make_set()
    ctx = _ctx(cs)
    t = build_traceability(
        contract_set_hash=ctx.entries["contract_set_hash"],
...
def test_traceability_differs_on_provider_change() -> None:
    ...
    assert t_a.provider is not None and t_b.provider is not None
